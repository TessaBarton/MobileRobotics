...
namespace DrRobot.JaguarControl
{
    public class Map
    {
        ...
        double GetWallDistance(double x, double y, double t, int segment){
            //args: the xyt location of the robot, a segment we could be looking at
            //outputs: expected range measurement if we are looking at the wall

            //equation of line for laser data
            double line_m = Math.tan( t);
            double line_b = y - ( line_m * x );
            // eqn range finder y = Math.tan(t) x + line_b, goes on forever
            // eqn line y = slopes[segment] x + intercepts[segment] limited by segmentsizes[segment]
            
            if(line_m = slopes[segment])
            {
                return 9999;
            }
            else
            {
                double intersectX = (line_b - intercepts[segment])/(line_m - slopes[segment]);
                double intersectY = intersectX * line_m + line_b;
                //check if intersect lies on wall
                ...
                if((intersectX<= maxX) && (intersectX>= minX) && (intersectX<= maxX) && (intersectX>= minX)){
                    return Math.sqrt( Math.Pow( x- intersectX, 2) + Math.Pow( y - intersectY, 2));
                }
                else{
                    return 9999;
                }
        ...
        public double GetClosestWallDistance(double x, double y, double t){
            ...
            for( int i = 0 ; i < slopes.length; i++)
                {
                if(GetWallDistance(x,y,t,i)< minDist)
                    mindist = GetWallDistance(x,y,t,i);
                    minWall = i;
                }
